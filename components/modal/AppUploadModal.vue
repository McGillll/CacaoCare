<template>
    ...
    >
        ...
        >
            <div
                v-if="isSaved"
                class="absolute top-4 right-4 sm:top-6 sm:right-6 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl shadow-lg flex items-center gap-2 sm:gap-3 z-60"
            >
                <CheckCircleIcon class="h-4 w-4 sm:h-5 sm:w-5" />
                <span class="text-sm sm:text-base font-medium">Download link saved successfully!</span>
            </div>
        </Transition>x``
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-2 sm:mx-4 overflow-hidden">
            ...
            >
                <div class="text-center flex w-full h-full flex-col justify-center p-4">
                    <div class="relative mx-auto mb-3 sm:mb-4">
                        <div class="animate-spin rounded-full h-10 w-10 sm:h-12 sm:w-12 border-4 border-green-200"></div>
                        <div class="animate-spin rounded-full h-10 w-10 sm:h-12 sm:w-12 border-4 border-green-600 border-t-transparent absolute top-0"></div>
                    </div>
                    <p class="text-gray-600 font-medium text-sm sm:text-base">Uploading app version...</p>
                    <p class="text-xs sm:text-sm text-gray-500">Please wait while we save your update</p>
                </div>
            </div>

            <!-- Header -->
            <div class="bg-gradient-to-r from-green-50 to-emerald-50 px-4 sm:px-8 py-4 sm:py-6 border-b border-green-100">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3 sm:gap-4">
                        <div class="p-2 sm:p-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl">
                            <CloudArrowUpIcon class="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                        </div>
                        <div>
                            <h3 class="text-lg sm:text-xl font-bold text-gray-900">Upload New Version</h3>
                            <p class="text-xs sm:text-sm text-gray-600">Add a new CacaoCare app update</p>
                        </div>
                    </div>
                    <button
                        @click="handleClose"
            ...
            <form @submit.prevent="handleSubmit" class="p-4 sm:p-8 space-y-4 sm:space-y-6">
                ...
                <div class="flex items-center justify-end gap-3 sm:gap-4 pt-4 border-t border-gray-100">
                    ...
                    >
                        <CloudArrowUpIcon class="h-3.5 w-3.5 sm:h-4 sm:w-4" />
