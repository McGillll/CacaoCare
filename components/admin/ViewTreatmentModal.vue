<template>
	<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50">
		<div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
			<!-- Header -->
			<div class="bg-gradient-to-r from-green-600 to-emerald-700 px-4 sm:px-6 py-3 sm:py-4">
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-2 sm:gap-3">
						...
						<div>
							<h2 class="text-lg sm:text-xl font-bold text-white">Treatment Protocol</h2>
							<p class="text-green-100 text-xs sm:text-sm">Complete management guide</p>
						</div>
					</div>
					<button
						@click="$emit('close')"
			...
			<!-- Content -->
			<div class="overflow-y-auto max-h-[calc(95vh-64px)] sm:max-h-[calc(90vh-80px)]">
				<!-- Treatment Overview -->
				<div class="p-3 sm:p-6 border-b border-gray-200">
					<div class="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4">
						<!-- Treatment Image -->
						<div class="relative w-20 h-20 sm:w-32 sm:h-32 rounded-xl overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 flex-shrink-0">
							<img
						...
						<div class="flex-1 text-center sm:text-left">
							<div class="flex flex-col sm:flex-row items-center sm:items-center gap-2 mb-3">
								<div class="p-1.5 bg-red-100 rounded-lg">
									<BugAntIcon class="h-5 w-5 text-red-600" />
								</div>
								<div>
									<h3 class="text-xl sm:text-2xl font-bold text-gray-900">{{ treatment.disease }}</h3>
									<p class="text-xs sm:text-sm text-gray-500 uppercase tracking-wide font-medium">Fungal Disease</p>
								</div>
							</div>
							
							<div class="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 mb-4">
							...
							<p class="text-gray-700 text-sm sm:text-base leading-relaxed line-clamp-3 sm:line-clamp-none">
				...
				<!-- Treatment Steps -->
				<div class="p-3 sm:p-6" v-if="treatment.keypoints && treatment.keypoints.length > 0">
					<div class="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
						<div class="p-1.5 sm:p-2 bg-green-100 rounded-lg">
							<ClipboardDocumentListIcon class="h-5 w-5 sm:h-6 sm:w-6 text-green-600" />
						</div>
						<div>
							<h4 class="text-lg sm:text-xl font-bold text-gray-900">Treatment Protocol Steps</h4>
							<p class="text-sm text-gray-600">Follow these steps for effective treatment</p>
						</div>
					</div>
					...
					<div class="space-y-2 sm:space-y-4">
						<div
							...
							:key="index"
							class="bg-gray-50 rounded-xl p-3 sm:p-4 border border-gray-200 hover:border-green-200 transition-colors duration-200"
						>
							<div class="flex items-start gap-3 sm:gap-4">
								...
								<!-- Step Content -->
								<div class="flex-1">
									<h5 class="text-base sm:text-lg font-semibold text-gray-900 mb-1 sm:mb-2">{{ keypoint.title }}</h5>
									<p class="text-sm sm:text-base text-gray-700 leading-relaxed">{{ keypoint.detailed_step }}</p>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="bg-gray-50 px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200">
					<div class="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-0">
						...
						<div class="flex justify-end">
							<button
								@click="$emit('close')"
								class="flex items-center gap-2 px-4 sm:px-6 py-2 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white rounded-lg transition-all duration-200 font-semibold shadow-md hover:shadow-lg text-sm sm:text-base"
							>
