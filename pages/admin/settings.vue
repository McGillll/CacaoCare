<template>
    ...
    <NuxtLayout @currentUser="handleUser" name="admin">
        ...
        <div class="mb-6 sm:mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
                <div class="flex-1">
                    ...
                    <p class="text-base text-gray-600">Manage your account preferences and information</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            <div class="p-4 sm:p-6 md:p-8">
                <form @submit.prevent="saveSettings" class="space-y-6 sm:space-y-8">
                    ...
                    <div class="flex flex-col sm:flex-row items-start gap-4 sm:gap-6 p-4 sm:p-6 bg-gradient-to-br from-gray-50 to-blue-50 rounded-xl border border-blue-100">
                        <div class="w-full sm:w-auto flex justify-center">
                            <div v-if="!state.newInfo.profile" class="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-gray-300 animate-pulse" />
                            <img v-else :src="state.newInfo.profile || ''" 
                        ...
                        <div class="flex-1 space-y-3 sm:space-y-4 w-full">
                            <div>
                                ...
                                <input id="profile" type="file" @change="handlePhotoUpload" accept="image/*" 
                            ...
                            <div v-if="state.profileData.name" 
                                class="flex items-center flex-wrap gap-2">
                                <span class="text-sm text-gray-600">Selected: {{ state.profileData.name }}</span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div class="space-y-2">
                            ...
                            <div v-if="state.newInfo.newPassword" class="mt-4 p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200">
                                <h4 class="flex items-center text-sm font-semibold text-gray-800 mb-3">
                                    <ShieldCheckIcon class="w-4 h-4 mr-2 text-blue-500" />
                                    Password Requirements
                                </h4>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <div class="flex items-center space-x-2">
                                        <div :class="isUpperValid ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'" 
                                        ...
                                        <span :class="isUpperValid ? 'text-green-700' : 'text-gray-600'" class="text-sm">
                                            Uppercase letter
                                        </span>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        <div :class="isLowerValid ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'" 
                                                class="w-5 h-5 rounded-full flex items-center justify-center transition-all duration-300">
                                            <CheckIcon v-if="isLowerValid" class="w-3 h-3" />
                                            <XMarkIcon v-else class="w-3 h-3" />
                                        </div>
                                        <span :class="isLowerValid ? 'text-green-700' : 'text-gray-600'" class="text-sm">
                                            Lowercase letter
                                        </span>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        <div :class="isSpecialCharValid ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'" 
                                                class="w-5 h-5 rounded-full flex items-center justify-center transition-all duration-300">
                                            <CheckIcon v-if="isSpecialCharValid" class="w-3 h-3" />
                                            <XMarkIcon v-else class="w-3 h-3" />
                                        </div>
                                        <span :class="isSpecialCharValid ? 'text-green-700' : 'text-gray-600'" class="text-sm">
                                            Special character
                                        </span>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        <div :class="isLengthValid ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'" 
                                                class="w-5 h-5 rounded-full flex items-center justify-center transition-all duration-300">
                                            <CheckIcon v-if="isLengthValid" class="w-3 h-3" />
                                            <XMarkIcon v-else class="w-3 h-3" />
                                        </div>
                                        <span :class="isLengthValid ? 'text-green-700' : 'text-gray-600'" class="text-sm">
                                            8+ characters
                                        </span>
                                    </div>
                                </div>
                        ...
                        <div class="space-y-2">
                            ...
                            />
                            <p v-if="!state.passwordMismatch && state.newInfo.newPassword && state.newInfo.confirmPassword === state.newInfo.newPassword" 
                                class="text-sm text-green-600 mt-1">Passwords match</p>
...
<script setup lang="ts">
    import { Cog6ToothIcon, ArrowLeftIcon, CheckIcon, ShieldCheckIcon, XMarkIcon } from '@heroicons/vue/24/outline'
