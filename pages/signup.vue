<template> 
    <NuxtLayout name="app">
        ...
        <section class="relative min-h-screen flex items-center bg-gradient-to-r overflow-x-hidden from-green-400 to-teal-500 text-white py-24 sm:py-16 px-8">
            ...
            <!-- FORM - Modern Multi-Step Design -->
            <div class="z-20 container mx-auto mt-8 max-w-4xl">
                <!-- Progress Indicator -->
                <div class="mb-8">
                    <div class="flex items-center justify-center space-x-4 mb-6">
                        <div class="flex items-center">
                            <div :class="currentStep >= 1 ? 'bg-green-500 text-white' : 'bg-white text-gray-400'" class="w-10 h-10 rounded-full flex items-center justify-center font-semibold border-2 border-white shadow-lg transition-all duration-300">
                                <UserIcon v-if="currentStep >= 1" class="w-5 h-5" />
                                <span v-else>1</span>
                            </div>
                            <span :class="currentStep >= 1 ? 'text-white' : 'text-white/60'" class="ml-2 font-medium hidden sm:inline">Personal</span>
                        </div>
                        <div class="w-12 h-1 bg-white/30 rounded-full">
                            <div :class="currentStep >= 2 ? 'w-full bg-white' : 'w-0 bg-white'" class="h-full rounded-full transition-all duration-500"></div>
                        </div>
                        <div class="flex items-center">
                            <div :class="currentStep >= 2 ? 'bg-green-500 text-white' : 'bg-white text-gray-400'" class="w-10 h-10 rounded-full flex items-center justify-center font-semibold border-2 border-white shadow-lg transition-all duration-300">
                                <MapPinIcon v-if="currentStep >= 2" class="w-5 h-5" />
                                <span v-else>2</span>
                            </div>
                            <span :class="currentStep >= 2 ? 'text-white' : 'text-white/60'" class="ml-2 font-medium hidden sm:inline">Location</span>
                        </div>
                        <div class="w-12 h-1 bg-white/30 rounded-full">
                            <div :class="currentStep >= 3 ? 'w-full bg-white' : 'w-0 bg-white'" class="h-full rounded-full transition-all duration-500"></div>
                        </div>
                        <div class="flex items-center">
                            <div :class="currentStep >= 3 ? 'bg-green-500 text-white' : 'bg-white text-gray-400'" class="w-10 h-10 rounded-full flex items-center justify-center font-semibold border-2 border-white shadow-lg transition-all duration-300">
                                <CheckIcon v-if="currentStep >= 3" class="w-5 h-5" />
                                <span v-else>3</span>
                            </div>
                            <span :class="currentStep >= 3 ? 'text-white' : 'text-white/60'" class="ml-2 font-medium hidden sm:inline">Complete</span>
                        </div>
                    </div>
                </div>

                <!-- Form Card -->
                <div class="relative bg-white/95 backdrop-blur-xl text-gray-800 p-8 md:p-12 rounded-3xl shadow-2xl border border-white/20">
                    <!-- Loading Overlay -->
                        ...
                        <div class="text-center">
                            <div class="relative">
                                ...
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <UserPlusIcon class="w-6 h-6 text-green-500 animate-pulse" />
                                </div>
                            </div>
                            <p class="mt-4 text-lg font-semibold text-gray-900">Creating Your Account</p>
                            <p class="text-sm text-gray-600">Please wait while we set up your farming profile...</p>
                        </div>
                    </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-8">
                        ...
                        <div v-if="currentStep === 1" class="space-y-6">
                            <div class="text-center mb-8">
                                ...
                                <h2 class="text-3xl font-bold text-gray-900 mb-2">Welcome to CacaoCare!</h2>
                                <p class="text-gray-600">Let's start by creating your account with basic information</p>
                            </div>

                            <div class="grid md:grid-cols-2 gap-6">
                                <!-- Username Field -->
                                <div class="space-y-2">
                                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2" for="username">
                                        <AtSymbolIcon class="w-4 h-4 mr-2 text-green-500" />
                                        Username
                                    </label>
                                    <div class="relative">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="mt-4 p-6 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl border border-gray-200">
                                    <h4 class="flex items-center text-sm font-semibold text-gray-800 mb-4">
                                        <ShieldCheckIcon class="w-4 h-4 mr-2 text-green-500" />
                                        Password Security Requirements
                                    </h4>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div class="flex items-center space-x-3">
                                            <div :class="isUpperValid ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'" class="w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300">
                                                <CheckIcon v-if="isUpperValid" class="w-3 h-3" />
                                                <XMarkIcon v-else class="w-3 h-3" />
                                            </div>
                                            <span :class="isUpperValid ? 'text-green-700 font-medium' : 'text-gray-600'" class="text-sm">
                                                Uppercase letter
                                            </span>
                                        </div>
                                        <div class="flex items-center space-x-3">
                                            <div :class="isLowerValid ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'" class="w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300">
                                                <CheckIcon v-if="isLowerValid" class="w-3 h-3" />
                                                <XMarkIcon v-else class="w-3 h-3" />
                                            </div>
                                            <span :class="isLowerValid ? 'text-green-700 font-medium' : 'text-gray-600'" class="text-sm">
                                                Lowercase letter
                                            </span>
                                        </div>
                                        <div class="flex items-center space-x-3">
                                            <div :class="isSpecialCharValid ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'" class="w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300">
                                                <CheckIcon v-if="isSpecialCharValid" class="w-3 h-3" />
                                                <XMarkIcon v-else class="w-3 h-3" />
                                            </div>
                                            <span :class="isSpecialCharValid ? 'text-green-700 font-medium' : 'text-gray-600'" class="text-sm">
                                                Special character
                                            </span>
                                        </div>
                                        <div class="flex items-center space-x-3">
                                            <div :class="isLengthValid ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'" class="w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300">
                                                <CheckIcon v-if="isLengthValid" class="w-3 h-3" />
                                                <XMarkIcon v-else class="w-3 h-3" />
                                            </div>
                                            <span :class="isLengthValid ? 'text-green-700 font-medium' : 'text-gray-600'" class="text-sm">
                                                8+ characters
                                            </span>
                                        </div>
                                    </div>
                            ...
                            <div class="flex justify-end pt-6">
                                <button
                                    ...
                                    class="flex items-center space-x-2 px-8 py-4 bg-gradient-to-r from-green-500 to-teal-500 text-white font-semibold rounded-2xl hover:from-green-600 hover:to-teal-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl"
                                >
                        ...
                        <div v-if="currentStep === 2" class="space-y-6">
                            <div class="text-center mb-8">
                                <div class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-teal-500 to-blue-500 rounded-2xl mb-4">
                                    <MapPinIcon class="w-8 h-8 text-white" />
                                </div>
                                <h2 class="text-3xl font-bold text-gray-900 mb-2">Farm Location</h2>
                                <p class="text-gray-600">Help us locate your cacao farm for better local support</p>
                            </div>
                            ...
                            <div class="p-6 bg-gradient-to-r from-green-50 to-teal-50 rounded-2xl border border-green-100">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <h3 class="font-bold text-gray-900">Quick Setup Available</h3>
                                            <p class="text-sm text-gray-600">Is your farm located in Davao City?</p>
                                        </div>
                                    </div>
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input
                                            ...
                                            @change="handleDavaoToggle"
                                            class="sr-only peer"
                                        />
                                        <div class="relative w-14 h-7 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-green-300/50 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-gradient-to-r peer-checked:from-green-500 peer-checked:to-teal-500"></div>
                                    </label>
                                </div>
                            </div>
                            ...
                            <!-- Location Dropdowns -->
                            <div class="space-y-6">
                                <!-- Region & Province (Non-Davao) -->
                                <div v-if="!state.fromDavao" class="grid md:grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        <label class="flex items-center text-sm font-medium text-gray-700 mb-2" for="region">
                                            <GlobeAltIcon class="w-4 h-4 mr-2 text-teal-500" />
                                            Region
                                        </label>
                                        <div class="relative">
                                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                            ...
                                            <select
                                                ...
                                                class="w-full pl-12 pr-4 py-4 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white text-gray-900"
                                                required
                                            >
                                    ...
                                    <div class="space-y-2">
                                        <label class="flex items-center text-sm font-medium text-gray-700 mb-2" for="province">
                                            <MapIcon class="w-4 h-4 mr-2 text-teal-500" />
                                            Province
                                        </label>
                                        <div class="relative">
                                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                ...
                                <!-- City & Barangay -->
                                <div class="grid md:grid-cols-2 gap-6">
                                    <div v-if="!state.fromDavao" class="space-y-2">
                                        <label class="flex items-center text-sm font-medium text-gray-700 mb-2" for="city">
                                            <BuildingOfficeIcon class="w-4 h-4 mr-2 text-teal-500" />
                                            City/Municipality
                                        </label>
                                        <div class="relative">
                                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    ...
                                    <div class="space-y-2" :class="state.fromDavao ? 'md:col-span-2' : ''">
                                        <label class="flex items-center text-sm font-medium text-gray-700 mb-2" for="barangay">
                                            <HomeIcon class="w-4 h-4 mr-2 text-teal-500" />
                                            Barangay
                                        </label>
                                        <div class="relative">
                                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            ...
                            <div class="flex justify-between pt-6">
                                <button
                                    type="button"
                                    @click="prevStep"
                                    class="flex items-center space-x-2 px-6 py-3 bg-gray-100 text-gray-700 font-medium rounded-xl hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 transition-all duration-200"
                                >
                                ...
                                <button
                                    ...
                                    class="flex items-center space-x-2 px-8 py-3 bg-gradient-to-r from-teal-500 to-blue-500 text-white font-semibold rounded-xl hover:from-teal-600 hover:to-blue-600 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl"
                                >
                        ...
                        <div v-if="currentStep === 3" class="space-y-6">
                            ...
                            <div class="flex justify-between pt-6">
                                ...
                                <button
                                    ...
                                    class="flex items-center space-x-3 px-8 py-4 bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold rounded-xl hover:from-blue-600 hover:to-purple-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl"
                                >
...
const isStep2Valid = computed(() => {
    return selectedBarangay.value.code;
});
...
function handleDavaoToggle(){
    if(state.fromDavao){
        ...
        selectedCity.value.code = ""
        selectedCity.value.name = ""
        selectedBarangay.value = {} as Barangay // Reset selected barangay
        barangays.value = []
        fetchRegions();
    }
}
